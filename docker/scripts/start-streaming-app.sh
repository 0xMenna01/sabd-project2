...
docker run -d --network project2_network --name flink-app --volume ./src/flink:/opt/flink/job --volume ./Results:/opt/flink/job/results jobsubmit-app bash -c "flink run -m jobmanager:8081 --jarfile /KafkaConnectorDependencies.jar --python /opt/flink/job/main.py $QUERY_NUM"

# Start producing events
#./docker/scripts/start-producing.sh
